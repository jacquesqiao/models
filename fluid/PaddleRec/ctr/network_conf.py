...
def ctr_dnn_model(embedding_size, sparse_feature_dim):
    ...
    datas = [label] + [dense_input] + sparse_input_ids
    ...
    #                                                   use_double_buffer=True)
    # words = fluid.layers.read_file(py_reader)
    ctr_reader = fluid.contrib.ctr_reader.ctr_reader(
        feed_dict=datas, file_type='plain', file_format='csv',
        file_list=['data/train_data.txt'], dense_slot_indexs=[1], sparse_slot_indexs=range(2, 28),
        capacity=64, thread_num=12, batch_size=1024, slots=[], name='ctr_reader')
    ...
    return avg_cost, auc_var, batch_auc_var, ctr_reader
